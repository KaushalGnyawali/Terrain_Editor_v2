...
"""
        ...
        # DEBUG: Show map_data structure when in basin mode
        if st.session_state.design_mode == "basin":
            with st.expander("🔍 DEBUG: map_data contents", expanded=True):
                if map_data is None:
                    st.warning("map_data is None")
                elif not map_data.get("all_drawings"):
                    st.warning("map_data has no 'all_drawings' key or it's empty")
                else:
            ...
            if geom_type == "LineString" and st.session_state.design_mode == "basin":
                ...
                if len(channel_coords) >= 2:
                    ...
                    if is_new_channel:
                        ...
                        st.session_state.basin_modified_dem = None
                        # Set flag to auto-zoom to channel on next rerun
                        st.session_state.channel_just_drawn = True
                        # Don't call st.rerun() - st_folium will trigger natural rerun
                        # when it detects the session_state change on the next interaction
